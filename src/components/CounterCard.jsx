            ...
            <div className="w-110 h-120 bg-[#2D2D2D] rounded-md shadow-2xl p-5 inter">
                ...
                <div className='flex flex-col gap-3 h-5/10 py-2'>
                    <div className='w-full h-3/7 text-white text-center flex flex-col gap-2'>
                        ...
                        <input className='w-full  bg-[#1A1A1A] border border-[#555555] text-center rounded-lg items-center h-full' type="number" value={step} onChange={(e) => setStep(Number(e.target.value))} />
                    </div>
                    <div className='flex justify-between gap-5 w-full h-2/7'>
                        <button onClick={() => setCount(count + step)} className='w-1/2 bg-[#39E079] h-full text-white rounded-lg text-[18px] font-bold transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_4px_12px_rgba(57,224,121,0.3)] cursor-pointer'>Increase</button>
                        <button onClick={() => setCount(count - step)} className='w-1/2 bg-[#1A4A2A] h-full rounded-lg text-[18px] font-bold text-[#39E079] transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-[0px_4px_20px_#4f735c] cursor-pointer'>Decrease</button>
                    </div>
                    <button onClick={() => {
                        setCount(0);
                        setStep(1);
